...
constexpr double nextV(const my_array<nx, ny>& p, const my_array<nx + 1, ny + 1>& v, std::size_t i, std::size_t j) {
    return 0.25 * (v[i+1][j] + v[i-1][j] + v[i][j+1] + v[i][j-1] + delta * delta * p[i][j] / epsilon );
}

my_array<nx + 1, ny + 1> fill_new_V(my_array<nx + 1, ny + 1>& act_v, my_array<nx + 1, ny + 1>& old_v, const my_array<nx, ny>& p, double wg) {
...
constexpr double calculate_S(const my_array<nx, ny>& p, const my_array<nx + 1, ny + 1>& v) {
    double result = 0.0;
    for (std::size_t i = 0; i < nx; i++) {
        for (std::size_t j = 0; j < ny; j++) {
            double temp = 0.0;
            temp += 0.5 * std::pow((v[i+1][j] - v[i][j]) / delta, 2);
            temp += 0.5 * std::pow((v[i][j+1] - v[i][j]) / delta, 2);
            temp -= p[i][j] * v[i][j];
            temp *= delta * delta;
            result += temp;
        }
...
double nextV_local(double wl, const my_array<nx, ny>& p, const my_array<nx+1, ny+1>& v, int i, int j) {
